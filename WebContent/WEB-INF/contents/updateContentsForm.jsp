	...
	function addDiv(layer) {
		...
		if(layer) {
	    	...
	    	$("#div").append("<div id='" + layer.id + "' style='overflow: hidden; width: " + layer.width + "px; height: " + layer.height + "px; background-color: " + layer.backgroundColor + "; border-style: solid; position: absolute; left: " + layer.x + "px; top: "+ layer.y +"px' onclick='divClick(\"" + layer.id + "\")'>" + layer.id + "</div>");
 			...
 			$("#" + layer.id).html("<img  src='upload/" + layer.fileName + "'  style='width:100%' />");
          ...
          <div class="t_w fix">
            ...
            <table class="tb tb_row tb_ip st01">
              ...
              <tbody>
                <tr>
                  <th>제목</th>
                  <td><input type="text" id="title" name="id" value="<%= contents.get("contents_title") %>"/></td>
                </tr>
                <tr>
                  <th class="v_t">메모</th>
                  <td>
                    <textarea id="content" name="content"><%= contents.get("contents_content") %></textarea>
                  </td>
                </tr>
                <tr>
                  <th>분할</th>
...
<%
	...
	for(int i = 0; i < advertiserList.size(); i++) {
		...
		if((Integer)(contents.get("advertiser_num")) == (Integer)(advertiser.get("advertiser_num"))) {
%>
                      <option value="<%= advertiser.get("advertiser_num") %>" selected="selected"><%= advertiser.get("advertiser_name") %></option>
<%				
		} else {
%>                    
...
%>                      
          ...
          <div class="btn_area_b">             
            <a href="javascript:updateContent()" class="btn btn_tranfer"><span>수정</span></a>              	
          </div>
