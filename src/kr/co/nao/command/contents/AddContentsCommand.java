...
public class AddContentsCommand implements Command {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) {
		...
		if(member == null) {
			...
			boolean result = dao.addContents(title, content, (String)member.get("id"), advertiser_num);
			
			if(result) {
				int contentsNum = dao.maxContentsNum();
				...
				for(int i = 0; i < layerId.length; i++) {
					...
					dao.addLayer(id, width, height, x, y, bGColor, priority, contentsNum, fileNum, text, persent);
				}
				
//				request.setAttribute("contentsNum", contentsNum);
				...
				action.setPath("WEB-INF/contents/addFileResult.jsp");
			}
		}
