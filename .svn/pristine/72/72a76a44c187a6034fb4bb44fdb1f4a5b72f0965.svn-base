package kr.co.nao.command.schedule;

import java.io.UnsupportedEncodingException;
import java.util.Map;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;

import kr.co.nao.common.ActionForward;
import kr.co.nao.common.Command;
import kr.co.nao.dao.ScheduleDAO;

/**
 * 스케줄 수정
 * 
 * @author	김관형
 * @since		2019.11.18
 * @version	1.0
 * 
 * <pre>
 * << 개정이력 >>
 * ------------------------------------------------------
 * 수정일				수정자				수정내용
 * ------------------------------------------------------
 */

public class UpdateScheduleCommand implements Command {

	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) {
		
		HttpSession session = request.getSession();
		Map<String, Object> member = (Map<String, Object>)session.getAttribute("member");
		
		ActionForward action = new ActionForward();

		if(member == null) {
			action.setRedirect(true);
			action.setPath("loginForm.nao");

		} else {
			try {
				request.setCharacterEncoding("UTF-8");

			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
			
			String schedule_num = request.getParameter("schedule_num");
			String schedule_date = request.getParameter("schedule_date");
			String schedule_start_time = request.getParameter("schedule_start_time");
			String machine_num = request.getParameter("machine_num");
			String contents_num = request.getParameter("contents_num");
			
			ScheduleDAO dao = new ScheduleDAO();
			boolean result = dao.updateSchedule(schedule_num, schedule_date, schedule_start_time, machine_num, contents_num);
			
			if(result) {
				action.setRedirect(true);
				action.setPath("scheduleList.nao");
				
			} else {
				action.setRedirect(false);
				action.setPath("WEB-INF/schedule/updateScheduleResult.jsp");
			}
		}
		
		return action;
	}
}
